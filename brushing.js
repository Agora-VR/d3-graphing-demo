...
async function getDataSets(stockSets, weatherSets) {
    return {
        stock: stockSets.map(async (set) => Object.assign(set, {data: await fetchCsv(`/stock/${set.ticker}.csv`)})),
        weather: weatherSets.map(async (set) =>  Object.assign(set, {data: await fetchCsv(`/weather/${set.name}.csv`)})),
    };
}
...
.then(([dataSets, weatherSets]) => {
    ...
    const getX = (d) => d.Date, getY = (d) => d.Close,
        getWX = (d) => d.DATE, getWY = (d) => d.TAVG;
    ...
    const focusHeight = 100,
        focusMargin = {top: 20, right: 20, bottom: 30, left: 40};
    ...
    function updateGraph(focusX, focusY, focusW) {
        ...
        gy.call(yAxis, focusY, "↑ Close $");
        gw.call(wAxis, focusW, "Temp");
    ...
    brush.on("brush", () => {
        ...
        updateGraph(
            x.copy().domain(d3.event.selection.map(x.invert, x).map(d3.utcDay.round)),
            y.copy(), w.copy());
    })
